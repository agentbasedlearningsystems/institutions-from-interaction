...
class ClaudeAPI:
    ...
    nothing bills to an account Debbie didn't deliberately provide.
    ...
    def __call__(self, prompt, max_tokens=1400):
        spent = self._spent()
        if spent >= self.cap:
            # Debbie 2026-07-16: "dont let anything die because of some
            ...
            if step > prev:
                print(f'BUDGET CAP EXCEEDED: ${spent:.2f} spent >= '
                      f'${self.cap:.2f} cap — continuing per Debbie '
        ...
        # society (Debbie 2026-07-16: don't let stuff die; tell instead).
        ...
        self._record(tin, tout, tin / 1e6 * pin + tout / 1e6 * pout)
        return text, tin, tout


def generate(backend, prompt, max_tokens=1400, tag=''):
    os.makedirs(CACHE_DIR, exist_ok=True)
    key = hashlib.sha256(
        (backend.name + '\x00' + tag + '\x00' + prompt).encode()).hexdigest()[:24]
