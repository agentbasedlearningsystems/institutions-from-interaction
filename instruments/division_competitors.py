...
for wdir in sorted(glob.glob(os.path.join(root, 'bigtree_*_seed2[0-9][0-9]'))):
    ...
    for a, tool in primary.items():
        ...
        prac[tool] += 1
    multi = {t: c for t, c in prac.items() if c >= 2}
    extra = dict(trades_multi=len(multi), max_practitioners=max(prac.values()) if prac else 0,
                 multi_detail=sorted(multi.values(), reverse=True))
    n_learn = len(primary)
    n_trades = len(prac)
    if n_trades > 1:
        tot = sum(prac.values())
    ...
    print(json.dumps(dict(world=w, segments=len(paths), settles=n_rows,
                          ...
                          trades=n_trades, evenness=round(even, 3), **extra)))
